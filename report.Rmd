...
riskFactors$factors <- gsub("NA &" , "",riskFactors$factors)
riskFactors$factors <- gsub(" & NA & ","", riskFactors$factors)
riskFactors <- riskFactors[which(riskFactors$factors =="EPOC" | riskFactors$factors ==" DIABETES" | riskFactors$factors ==" DIALISIS" | riskFactors$factors ==" OBESIDAD.MORBIDA"),]
...
#Pie
pie(percentages,main = "Men and women percentage", labels = c(paste("Women", percentageOfFemale,"%"), paste("Men", percentageOfMale,"%")), theta = 1, explode = 0.00, col = c("pink","light blue"))
...
#Histograma
g <- ggplot(dataset, aes(x = EDAD, fill = SEXO)) +
  geom_density(alpha = 0.4) +
  scale_fill_discrete(name = "Gender", labels = c("Female","Male")) +
  xlab("Age")+
  ylab(" ")+
  theme(legend.position = "top")+
  geom_vline(data=dataset, aes(xintercept=ageAverageInMen, color = "Male"),
               linetype="dashed")+
  geom_vline(data=dataset, aes(xintercept=ageAverageInWomen, color = "Female"),
             linetype="dashed")

g <- g+annotate(x=ageAverageInMen + 1,y=0.028,label=round(ageAverageInMen,2),vjust=2,geom="label")
g+annotate(x=ageAverageInWomen -1,y=0.025,label=round(ageAverageInWomen,2),vjust=2,geom="label")
...
pie(percentageByProcedureArray, main = "Percentage by procedure",
      ...
      theta = 1,
      explode = 0.00, col = c("pink","light blue", "light green"))
...
## Number of pacients with factors of risk {.tabset .unnumbered}
...
## Percentage of risk and no risk per procedure (without endovalvula)
```{r,echo = FALSE, message = FALSE, warning = FALSE,fig.width=14}
